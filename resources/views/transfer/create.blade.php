    ...
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form class="form-sample" action="{{ route('transfer.store') }}" method="POST"
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="vehicle_id" id="vehicle_id">
                                        @foreach ($vehicles as $id => $value)
                                            <option {{ $id == old('vehicle_id') ? 'Selected' : '' }}
                                                    value="{{ $id }}">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="driver_id" id="driver_id">
                                        @foreach ($drivers as $id => $value)
                                            <option {{ $id == old('driver_id') ? 'Selected' : '' }}
                                                    value="{{ $id }}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="passenger_id" id="passenger_id">
                                        @foreach ($passengers as $id => $value)
                                            <option {{ $id == old('passenger_id') ? 'Selected' : '' }}
                                                    value="{{ $id }}">
                                                {{ $value->name }} - {{$value->phone}}</option>
                                        @endforeach
                                    </select>
